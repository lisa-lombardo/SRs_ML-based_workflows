...
def main():
    ...
    joblib.dump(X.columns, os.path.join(args.output_dir, "selected_features.joblib"))
    ...
    def objective(trial):
        clf = SVC(
            kernel='rbf',
            C=trial.suggest_float('C', 0.57, 1.10, log=True),
            gamma=trial.suggest_float('gamma', 5.3e-4, 1.4e-3, log=True),
            class_weight='balanced',
            probability=True
        )
        pipeline = Pipeline([('scaler', StandardScaler()), ('classifier', clf)])
        auc_scores = cross_val_score(pipeline, X, y, cv=stratkfold, scoring='roc_auc', n_jobs=-1)
    ...
    for train_idx, test_idx in stratkfold.split(X, y):
        ...
        results_df.loc[len(results_df)] = [
            fold, auc, f1, precision, recall, mcc, cm.tolist(), {}, {}, {}, {}
        ]
    ...
    pipeline.fit(X, y)
    joblib.dump(pipeline, os.path.join(models_dir, "svm_final_pipeline_s1r.joblib"))
    print(f"Final model saved to {models_dir}/svm_final_pipeline_s1r.joblib")
