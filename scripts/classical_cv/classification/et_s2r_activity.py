...
def main():
    ...
    def objective(trial):
        clf = ExtraTreesClassifier(
            n_estimators=trial.suggest_int('n_estimators', 269, 809),
    ...
    pipeline.fit(X, y)
    joblib.dump(pipeline, os.path.join(models_dir, "et_final_pipeline_s2r.joblib"))
    print(f"Final model saved to {models_dir}/et_final_pipeline_s2r.joblib")
