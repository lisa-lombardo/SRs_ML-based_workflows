...
def main():
    ...
    clf_best = XGBClassifier(**best_params, n_jobs=3)
    ...
    for train_idx, test_idx in stratkfold.split(X, y):
        ...
        joblib.dump(pipeline, os.path.join(models_dir, f"xgb_pipeline_fold_{fold}.joblib"))
    ...
    pipeline.fit(X, y)
    joblib.dump(pipeline, os.path.join(models_dir, "xgb_final_pipeline_s1r.joblib"))
    print(f"Final model saved to {models_dir}/xgb_final_pipeline_s1r.joblib")
