...
def main():
    parser = argparse.ArgumentParser(description="Nested CV for ExtraTreesRegressor on multiple descriptor sets.")
    ...
    for descriptor_name, df in datasets.items():
        ...
        for outer_fold, (outer_train_idx, outer_test_idx) in enumerate(outer_cv.split(X, y), start=1):
            ...
            def objective(trial):
                ...
                # Perform inner cross-validation for both RMSE and R²
                scoring = {'rmse': make_scorer(rmse_scorer), 'r2': 'r2'}
                cv_results = cross_validate(pipeline, X_train_outer, y_train_outer, cv=inner_cv, scoring=scoring)
