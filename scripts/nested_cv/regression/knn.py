...
def main():
    parser = argparse.ArgumentParser(description="Nested CV for KNeighborsRegressor on multiple descriptor sets.")
    ...
    for descriptor_name, df in datasets.items():
        ...
        for outer_fold, (outer_train_idx, outer_test_idx) in enumerate(outer_cv.split(X, y), start=1):
            ...
            def objective(trial):
                ...
                # Create the KNeighborsRegressor with suggested hyperparameters
                knn = KNeighborsRegressor(n_neighbors=n_neighbors, weights=weights, metric=metric, algorithm=algorithm, leaf_size=leaf_size, n_jobs=3)
            ...
            best_model = KNeighborsRegressor(**best_params, n_jobs=3)
            pipeline = Pipeline([('scaler', StandardScaler()), ('knn', best_model)])
