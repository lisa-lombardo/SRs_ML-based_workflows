...
def main():
    parser = argparse.ArgumentParser(description="Nested CV for SVC on multiple descriptor sets.")
    ...
    for descriptor_name, df in datasets.items():
        print(f"\nSVC: Performing nested cross-validation for {descriptor_name} descriptor set...")
        ...
        for outer_train_idx, outer_test_idx in outer_cv.split(X, y):
            ...
            def objective(trial, X_resampled, y_resampled):
                ...
                # Create the SVC with suggested hyperparameters
                svc = SVC(kernel='rbf', C=C, gamma=gamma, class_weight=class_weight, probability=True, decision_function_shape='ovr')
                ...
                for inner_train_idx, inner_test_idx in inner_cv.split(X_resampled, y_resampled):
                    ...
                    svc.fit(X_train_inner_scaled, y_train_inner)
                    y_pred_inner = svc.predict_proba(X_valid_inner_scaled)
            ...
            best_model = SVC(**best_params, probability=True, decision_function_shape='ovr')
