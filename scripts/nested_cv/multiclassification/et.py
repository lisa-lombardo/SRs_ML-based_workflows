...
def main():
    parser = argparse.ArgumentParser(description="Nested CV for ExtraTrees on multiple descriptor sets.")
    ...
    for descriptor_name, df in datasets.items():
        print(f"\nExtraTrees: Performing nested cross-validation for {descriptor_name} descriptor set...")
        ...
        for outer_train_idx, outer_test_idx in outer_cv.split(X, y):
            ...
            def objective(trial, X_resampled, y_resampled):
                # Suggest hyperparameters for ExtraTreesClassifier
                ...
                # Create the ExtraTreesClassifier with suggested hyperparameters
                etc = ExtraTreesClassifier(n_estimators=n_estimators, max_depth=max_depth, 
                                           min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, 
                                           bootstrap=bootstrap, class_weight=class_weight, random_state=1, n_jobs=3)
                ...
                for inner_train_idx, inner_test_idx in inner_cv.split(X_resampled, y_resampled):
                    ...
                    etc.fit(X_train_inner_scaled, y_train_inner)
                    y_pred_inner = etc.predict_proba(X_valid_inner_scaled)
            ...
            X_train_outer_scaled = np.clip(X_train_outer_scaled, -1e5, 1e5)
            X_test_outer_scaled = np.clip(X_test_outer_scaled, -1e5, 1e5)

            # Fit the model
            best_model = ExtraTreesClassifier(**best_params, random_state=1, n_jobs=3)
        ...
        # Compute mean for each outer metric
