...
def main():
    parser = argparse.ArgumentParser(description="Nested CV for RandomForestClassifier on multiple descriptor sets.")
    ...
    for descriptor_name, df in datasets.items():
        ...
        for outer_train_idx, outer_test_idx in outer_cv.split(X, y):
            ...
            scaler = StandardScaler()
            X_train_outer_scaled = scaler.fit_transform(X_train_outer_resampled).astype(np.float64)
            X_test_outer_scaled = scaler.transform(X_test_outer).astype(np.float64)
            ...
            def objective(trial, X_resampled, y_resampled):
                ...
                clf = RandomForestClassifier(
                    bootstrap=bootstrap, max_features=max_features, n_estimators=n_estimators, max_depth=max_depth,
                    min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, class_weight=class_weight, n_jobs=3
                )
                ...
                for inner_train_idx, inner_test_idx in inner_cv.split(X_resampled, y_resampled):
                    ...
                    clf.fit(X_train_inner_scaled, y_train_inner)
                    y_pred_inner = clf.predict_proba(X_valid_inner_scaled)
