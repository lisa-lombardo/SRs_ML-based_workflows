...
def main():
    parser = argparse.ArgumentParser(description="Nested CV for XGBClassifier on multiple descriptor sets.")
    ...
    for descriptor_name, df in datasets.items():
        ...
        for outer_train_idx, outer_test_idx in outer_cv.split(X, y):
            ...
            def objective(trial, X_resampled, y_resampled):
                ...
                # Create the XGBClassifier with suggested hyperparameters
                ...
                for inner_train_idx, inner_test_idx in inner_cv.split(X_resampled, y_resampled):
                    ...
                    xgb.fit(X_train_inner_scaled, y_train_inner)
                    y_pred_inner = xgb.predict_proba(X_valid_inner_scaled)[:, 1]

                    # Calculate AUC for the validation set
                    auc_score = roc_auc_score(y_valid_inner, y_pred_inner)
                    auc_scores.append(auc_score)

                avg_auc = np.mean(auc_scores)
                trial.set_user_attr('auc', avg_auc)
                return avg_auc  # Optimize AUC
            ...
            # Standardize the outer training and test sets
            scaler = StandardScaler()
            X_train_outer_scaled = scaler.fit_transform(X_train_outer_resampled)
            X_test_outer_scaled = scaler.transform(X_test_outer)

            # Calculate sample weights
            sample_weights = compute_sample_weight(class_weight='balanced', y=y_train_outer_resampled)

            # Fit the model with sample weights
            best_model = XGBClassifier(**best_params, eval_metric='logloss', random_state=1)
            best_model.fit(X_train_outer_scaled, y_train_outer_resampled, sample_weight=sample_weights)
        ...
        print(f"Mean AUC (outer): {mean_auc_outer:.4f} ± {std_auc_outer:.4f}")
        print(f"Mean F1 (outer): {mean_f1_outer:.4f} ± {std_f1_outer:.4f}")
        ...
        results_df.loc[results_df['Descriptor'] == descriptor_name, 'Mean AUC (outer loop)'] = mean_auc_outer
        results_df.loc[results_df['Descriptor'] == descriptor_name, 'Mean F1 (outer loop)'] = mean_f1_outer
